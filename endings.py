...
def run():
    ...
    def update_screen():
        ...
        heading = myfont.render('Word Family Endings', False, WHITE)
    ...
    reader = csv.DictReader(open('endings.csv', 'r'))
    dict_list = []
    for line in reader:
    ...
    while not done:
        ...
        while not play:
            ...
            ending = (dict_list[active_word_index]["ending"])
            blank_sentence = random_word.replace(ending, '_ _')
            wordaslist = list(ending) 	
            update_screen()
            while letters_revealed <len(ending):    
